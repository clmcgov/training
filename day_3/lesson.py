...
array([[-116, -123, -120, ...,  -75,  -80,  -94],
    ...
    Funky -- everything that was less than 127 stayed the same, while everything
...
True
    ...
    make each one three times bigger than it needs to be -- I know I am. Let's
    dig deeper. We have six bands, so if each one is duplicated three times,
    the size of our images should be something like:
...
...     mode='RGB',
...     bands=(
...         mtif[BANDS.index('red')].getchannel(0),
...         mtif[BANDS.index('green')].getchannel(0),
...         mtif[BANDS.index('blue')].getchannel(0)))

#>>> rgb.show()


    Same for false color

>>> cir = Image.merge(
...     mode='RGB',
...     bands=(
...         mtif[BANDS.index('nir')].getchannel(0),
...         mtif[BANDS.index('red')].getchannel(0),
...         mtif[BANDS.index('green')].getchannel(0)))
...
>>> red = np.array(mtif[BANDS.index('red')].getchannel(0))

>>> nir = np.array(mtif[BANDS.index('nir')].getchannel(0))
...
>>> good = timeit('red * red', number=10, globals=globals())
    ...
    the rows and columns. Fundamentally, the result is the same, but the the
    approach is vastly different. Remember that a list is a collection of
    references to other things that could be anything.
...
array([1, 2])
    ...
    don't want to include nan, so we exclude both nan and if from the
    calculation.
    ...
    There where function says: where arg 0 is True, make the value arg 1; where
